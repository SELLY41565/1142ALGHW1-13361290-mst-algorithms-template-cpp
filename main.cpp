...
#include <queue>
#include <algorithm>
#include <tuple> // 補上 tuple 標頭檔以供 Prim 演算法使用
using namespace std;
...
public:
    ...
    bool unite(int a, int b) {
        ...
        // 3. 否則合併兩集合 (使用 Union by Rank)，回傳 true
...
void primMST(int n, const vector<vector<pair<int, int>>>& adj, int start = 1) {
    ...
    for (const auto& neighbor : adj[start]) {
        pq.push({neighbor.second, start, neighbor.first}); // {w, from, to}
    }
